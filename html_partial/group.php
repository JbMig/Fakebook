...
<h1 id="h1"><img id="profilPic" src="img_groups_groups/<?= $group["picture"] ?>" alt="" width="40px"><span id="profileName"><?=$h1?></span></h1>
<!-- stats -->
<section> 
			...
			<?php if ($is_banned === false) : ?>
				...
				<?php foreach ($articles as $article): ?>
					...
					<div id="article" style="margin-top:20px; border: solid 1px black; padding: 10px; width: 500px">
						<form id="goTogroup" action="/public_group" method="post"> <!-- needs to be modified to match a group -->
							<input type="hidden" name="group_id" value="<?= $group_id ?>" />
							<button type="submit" class="articleColor" id="profil_picture" style="border:0; padding:5px;">
								<img id="profilPic" src="img_groups_groups/<?= $group["picture"] ?>" alt="" width="40px">
							</button>
				...
				<?php endforeach;?>
			<?php else: ?>
				<span>Vous avez été banni(e) de cette group. Vous ne pouvez donc plus voir son contenu.</span>
			<?php endif; ?>
		</div>
	</div>
	<div>
